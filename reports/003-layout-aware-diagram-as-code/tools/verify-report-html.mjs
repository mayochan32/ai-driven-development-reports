...
console.error("verify-report: launching browser");
const browser = await puppeteer.launch({ headless: true, timeout: 15_000 });
const results = [];
for (const viewport of [{ name: "desktop", width: 1440, height: 1000 }, { name: "mobile", width: 390, height: 844 }]) {
  console.error(`verify-report: checking ${viewport.name}`);
  ...
  console.error(`verify-report: ${viewport.name} page loaded`);
  const tabStates = [];
  for (const target of Object.keys(sourceFiles)) {
    await page.evaluate((selected) => {
      document.querySelector(`[data-compare-target="${selected}"]`).click();
    }, target);
  ...
  console.error(`verify-report: ${viewport.name} comparison tabs passed`);
  const variantTabStates = [];
  for (const target of Object.keys(variantSourceFiles)) {
    await page.evaluate((selected) => {
      document.querySelector(`[data-variant-target="${selected}"]`).click();
    }, target);
  ...
  console.error(`verify-report: ${viewport.name} variant tabs passed`);
  ...
  console.error(`verify-report: ${viewport.name} content inspected`);
  ...
  await page.close();
  console.error(`verify-report: ${viewport.name} passed`);
}
await browser.close();
console.error("verify-report: browser closed");
