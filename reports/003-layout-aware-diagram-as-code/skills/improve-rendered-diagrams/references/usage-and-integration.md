...
このガイド内の相対パスとコマンドは、`003-layout-aware-diagram-as-code/`テーマフォルダを作業ルートとする。同フォルダにはスキル、エージェント定義、レンダラー、評価基準がすべて同梱されている。
...
| Claude Code用 | `.claude/agents/improve-rendered-diagrams.md` | 同梱スキルを参照する専門サブエージェント |
...
このテーマフォルダには、エージェント定義を `.claude/agents/improve-rendered-diagrams.md`、共通スキルの正本を `skills/improve-rendered-diagrams/` に同梱している。テーマフォルダを作業ルートにすれば、エージェントは正本を相対参照できる。

Claude Codeのスキルとして自動プリロードしたい場合は、共通スキルを `.claude/skills/improve-rendered-diagrams/` または `~/.claude/skills/improve-rendered-diagrams/` へコピーする。その場合だけ、既存エージェントのfrontmatterへスキル名を追加する。
...
このテーマフォルダには、エージェント定義を `.github/agents/improve-rendered-diagrams.agent.md`、共通スキルの正本を `skills/improve-rendered-diagrams/` に同梱している。Copilot CLIでテーマフォルダを作業ルートにすると、エージェント定義から正本を参照できる。

Copilotのプロジェクトスキルとして登録する場合は、共通スキルを `.github/skills/improve-rendered-diagrams/` へコピーする。個人利用では `~/.copilot/skills/` と `~/.copilot/agents/` を使用する。
